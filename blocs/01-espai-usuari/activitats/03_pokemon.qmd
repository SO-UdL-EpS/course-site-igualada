---
title: "Exercicis amb C · Pokémon"
subtitle: "Activitat per casa · struct, typedef, memòria dinàmica i .h/.c"
description: "Activitat guiada per reforçar conceptes de C amb l'exemple d'un Pokémon: struct, typedef, punters, malloc/free/realloc, opaque types i Makefile."
lang: ca
---
...
::: {.prerequisits}
- Punters i aritmètica de punters, decaïment d'arrays
- `malloc`/`free` i *ownership* de memòria dinàmica
- `struct`, `typedef` i opaque types amb `.h`/`.c`
- `.h` = interfície / `.c` = implementació, `Makefile`
:::

## Estructures de dades en C

Un Pokémon el podem entendre com una estructura de dades que conté diferents camps. En aquest cas, els camps que ens interessen són:

* **pokemon_id**: identificador únic del Pokémon
* **name**: nom del Pokémon
* **height**: altura del Pokémon
* **weight**: pes del Pokémon
...
Podem fer un programa molt senzill per crear un Pokémon i mostrar-lo per pantalla.
...
En aquesta primera versió hem utilitzat una mida estàtica pel camp **name** utilitzant la *stack*. Això vol dir que el nom del Pokémon no pot ser més gran de 50 caràcters. També indica que estem desaprofitant memòria en tots els noms de Pokémon inferiors a 50 caràcters. Recordeu que la memòria és un recurs molt valuós i que hem d'aprofitar al màxim.

Per tant, per poder solucionar aquest problema, podem utilitzar la *heap* per reservar memòria dinàmicament per al camp **name**. Això ens permetrà utilitzar la memòria de forma més eficient i no tindrem cap limitació en la mida del nom del Pokémon. D'aquesta manera podem garantir que cada nom ocupi l'espai que requereixi.
...
::: {.bonapractica}
"Pikachu" té 7 caràcters, i aquí hem reservat exactament 8 — el vuitè byte és pel caràcter de final de cadena **'\0'**. Quan reserveu memòria per a una cadena, sempre heu de comptar aquest byte extra. En lloc de comptar caràcters a mà (fràgil: us heu d'equivocar un sol cop), la forma idiomàtica és:
:::
...
* ```pikachu.name = &name;```: **&name** és l'adreça de l'array *name* (de tipus `char (*)[8]`), i **pikachu.name** espera un `char *` — tipus incompatibles. Aquesta assignació **no és correcta**.

::: {.errorcomu}
Aquí hi ha una trampa clàssica de C: si ho compileu, `gcc` **no** ho rebutja — només emet un avís (`warning: assignment to 'char *' from incompatible pointer type`), i el programa fins i tot s'executa i imprimeix "Pikachu" correctament. Per què? Perquè **l'adreça d'un array i l'adreça del seu primer element són numèricament idèntiques** (`&name == &name[0] == name`). El valor numèric "funciona" per coincidència, però el tipus continua sent incorrecte — i confiar-hi és fràgil: no doneu per bo un codi només perquè compila sense error i sembla funcionar.
:::
...
::: {.definicio}
**malloc** i **calloc** ens permeten reservar memòria dinàmicament. La diferència és que `malloc` no inicialitza la memòria reservada (conté brossa), mentre que `calloc` la inicialitza tota a 0.
:::

Tota l'estructura de dades **pikachu** es troba a la **heap**: tant l'*id*, *height*, *weight* com el punter *name*. En canvi, si en algun moment fem servir un punter `Pokemon *pikachu` (com veurem més endavant amb `create_pokemon`), el propi punter `pikachu` viurà a la **stack**, apuntant a l'estructura sencera, que llavors sí que viurà tota ella a la **heap**.

## Ús de typedef

Ara podem utilitzar **typedef** per definir un nou tipus de dades que ens permeti crear Pokémon de forma més senzilla.
...
::: {.definicio}
**strdup** fa exactament el mateix que fer servir `malloc` + `strcpy`, però en una sola línia de codi. Com que reserva memòria, també cal alliberar-la amb `free`.
:::

## Creació i ús de llibreries

Per poder reutilitzar el codi que hem creat fins ara, podem crear una llibreria que ens permeti fer operacions amb Pokémon. Per exemple, podem moure la definició de la nostra estructura de dades **Pokemon** a un fitxer anomenat **pokemon.h** i la implementació de les funcions a un fitxer anomenat **pokemon.c**.
...
::: {.bonapractica}
`pokemon->name[0] = '\0';` no és decoratiu — és necessari. `malloc` **no** inicialitza la memòria que reserva; sense aquesta línia, `pokemon->name` conté brossa, sense cap `'\0'` garantit dins dels 10 bytes reservats. Unes línies més avall, `set_name` fa `strlen(pokemon->name)` — sobre memòria no inicialitzada, això és un accés fora de límits real (comprovat amb `AddressSanitizer`: `heap-buffer-overflow` en la primera crida a `set_name`), no només una possibilitat teòrica.
:::
...
::: {.concepte}
Ara que `create_pokemon()` garanteix que `pokemon->name` comença sempre amb un `'\0'` vàlid, aquest `strlen(pokemon->name)` ja és segur: la primera vegada donarà 0, no brossa.
:::
...
Imagineu que ara voleu guardar una llista o vector de Pokémon. Com ho faríeu? Una manera seria crear un vector de punters a la nostra estructura de dades **Pokemon**.
...
::: {.reflexio}
`Pokemon *pokemons[3]` és un array de punters a `Pokemon`. Compareu-ho amb el `Node **` de la *linked list* de la Sessió 4, o amb l'`int **` del Pralab 04: mateix patró d'indirecció, aplicat aquí a un tipus propi en lloc d'`int`.
:::

Podem complicar una mica més el disseny. Ara imagineu que voleu crear una llista dinàmica de Pokémon, on un usuari us pot demanar quants Pokémon vol introduir i després introduir les dades dels Pokémon en temps d'execució. Com ho faríeu?
...
D'aquesta manera podem crear una llista dinàmica de Pokémon i introduir les dades en temps d'execució. Per exemple:
...
::: {.errorcomu}
`scanf("%s", name)` és insegur, ja que pot causar un **buffer overflow** si l'usuari introdueix una cadena més llarga que la mida del buffer (aquí, 100 bytes). Per evitar aquest problema, es pot utilitzar `fgets()` en lloc de `scanf()`, ja que permet especificar la mida del buffer i evita el *buffer overflow*.
:::

## Proposta d'exercicis d'ampliació

::: {.repte}
1. Implementa el codi aplicant totes les millores per fer-lo més robust i segur: `strncpy` (o equivalent) en lloc de `strcpy` als setters, `fgets` en lloc de `scanf("%s", ...)`, i comprovació de tots els retorns de `malloc`/`realloc`.
...
3. Implementa una nova llibreria `pokedex.h` i `pokedex.c` que contingui les funcions per gestionar una Pokedex.
   ...
   - Utilitza `malloc` i `realloc` per gestionar el creixement de l'array dinàmic.
:::
